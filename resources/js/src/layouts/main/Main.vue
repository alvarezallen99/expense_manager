  ...
  <v-app id="inspire">
    ...
    >
      ...
      <v-list dense>
        ...
        <v-list-item class="mb-n5 ml-1" two-line :class="miniVariant && 'px-0'">
            <v-list-item-avatar>
            ...
            <v-subheader class="ml-4">{{ this.getSecureLocalStorage().name }} ({{this.getSecureLocalStorage().role_name}})</v-subheader>
        <v-divider></v-divider>
        ...
        <v-list-item
          ...
          >
            <v-subheader :class="item.type == 'header'? null:'ml-5'">{{item.title}}</v-subheader>
        </v-list-item>
        <v-list-item
            v-if="!isAdmin"
...
<script>
  export default {
    ...
    created(){
      if(this.getSecureLocalStorage().role == 1){
        this.isAdmin = true
      }else{
        this.isAdmin = false
      }
