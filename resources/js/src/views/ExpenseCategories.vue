    ...
    <v-container>
        ...
        <v-card outlined>
            ...
            <v-card-actions>
                ...
                <v-btn class="primary mr-3" outlined small dark @click="showaddCategoryForm()">Add Category</v-btn>
            </v-card-actions>
        </v-card>
            ...
            <v-dialog v-model.trim="addDialog" persistent max-width="600px" dense>
                <v-card >
                    ...
                    <v-form ref="form" :lazy-validation="true">
                        <v-card-text class="mt-n5">
                            <v-text-field
                                ...
                                :rules="[() => !!addForm.category_name || 'This field is required']"
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" @click="addCategory()" small dark width="75px">SAVE</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
            ...
            <v-dialog v-model.trim="updateDialog" persistent max-width="600px" dense>
                <v-card >
                    ...
                    <v-form ref="updateform" :lazy-validation="true">
                        <v-card-text class="mt-n5">
                            <v-text-field
                                ...
                                :rules="[() => !!updateForm.category_name || 'This field is required']"
                                :error-messages="errorMessages"
                                label="Display Name"
                                placeholder="Display Name"
                                required
                    ...
                    <v-card-actions>
                        ...
                        <v-spacer></v-spacer>
                        <v-btn color="warning" @click="cancelRoleForm()" small dark width="75px" outlined>CANCEL</v-btn>
                        <v-btn color="primary" @click="updateCategory()" small dark width="75px">UPDATE</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
...
export default {
    ...
    methods: {
        getData(){
            this.$http.post(`${this.url}/v1/categories`)
            .then(res => {
        ...
        deleteCategory(){
            this.$http.post(`${this.url}/v1/delete_category`, this.updateForm)
            .then(res => {
                let data = res.data.message
        ...
        addCategory(){
            this.$http.post(`${this.url}/v1/add_category`, this.addForm)
            .then(res => {
                let data = res.data.message
                this.tbldata.push(data[0])
                this.addDialog = false
                this.$toast.success('Successfully Added Expense Category!') 
            })
            .catch(err => {
                if(err.response.data.message != undefined){
                    this.$toast.warning(err.response.data.message);
        ...
        updateCategory(){
            this.$http.post(`${this.url}/v1/update_category`, this.updateForm)
            .then(res => {
                let data = res.data.message
                for(let x = 0; x < this.tbldata.length; x++){
                ...
                this.updateDialog = false
                this.$toast.success('Successfully Updated Expense Category!') 
            })
            .catch(err => {
                if(err.response.data.message != undefined){
                    this.$toast.warning(err.response.data.message);
